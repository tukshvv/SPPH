        ...
        <div class="rounded-2xl bg-white p-6 shadow ring-1 ring-orange-100">
          ...
          <div class="mt-4 grid gap-3 sm:grid-cols-2">
            ...
            <div class="stat-card">
              ...
              <p class="text-2xl font-bold text-orange-600">{{ dashboard?.totalDaysActive ?? 0 }}</p>
            </div>
            <div class="stat-card">
              <p class="text-xs text-slate-500">Current streak</p>
              <p class="text-2xl font-bold text-orange-600">{{ dashboard?.streakCurrent ?? 0 }} days</p>
            </div>
          </div>
        </div>
        <ActivityCalendar v-if="usageDays.length" :days="usageDays" />
      </div>
    </div>
...
const usageDays = computed(() => usage.data?.days ?? []);
</script>
