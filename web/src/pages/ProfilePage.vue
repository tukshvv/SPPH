      ...
      <aside class="space-y-6">
        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div class="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
            ...
            <div class="mt-3 flex flex-wrap gap-2">
              ...
              <button
                type="button"
                class="inline-flex items-center justify-center rounded-full border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-600 transition hover:bg-slate-100"
                @click="handleLogout"
              >
                Выйти из аккаунта
              </button>
        ...
        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div class="flex items-start justify-between gap-4">
            <div>
              <h2 class="text-lg font-semibold text-slate-900">Активность</h2>
              <p class="mt-1 text-sm text-slate-500">
                Частота взаимодействий за последние 52 недели. Оранжевые квадраты показывают более активные дни.
              </p>
            </div>
            <span v-if="totalActivityCount" class="rounded-full bg-orange-50 px-3 py-1 text-xs font-semibold text-orange-600">
              {{ totalActivityCount }}
            </span>
          </div>

          <div class="mt-4">
            <div v-if="metricsStore.statsLoading" class="h-24 animate-pulse rounded-2xl bg-slate-100"></div>
            <div v-else-if="metricsStore.statsError" class="rounded-2xl bg-rose-50 p-4 text-sm text-rose-500">
              {{ metricsStore.statsError }}
            </div>
            <div v-else-if="activityWeeks.length > 0" class="overflow-x-auto pb-2">
              <div class="flex items-start gap-[3px]">
                <div v-for="(week, weekIndex) in activityWeeks" :key="weekIndex" class="flex flex-col gap-[3px]">
                  <div
                    v-for="day in week"
                    :key="day.date"
                    class="h-3 w-3 rounded-[3px] transition"
                    :class="activityColor(day.count)"
                    :title="activityTitle(day.date, day.count)"
                  ></div>
                </div>
              </div>
            ...
            <div v-else class="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-4 text-center text-xs text-slate-500">
              Активность появится после первых запросов.
            </div>
          </div>
        </div>
...
const activityLevels = [
  { label: '0', className: 'bg-slate-200/80' },
  { label: '1-2', className: 'bg-orange-100' },
  { label: '3-4', className: 'bg-orange-200' },
  { label: '5-7', className: 'bg-orange-300' },
  { label: '8-10', className: 'bg-orange-400' },
  { label: '11+', className: 'bg-orange-500' }
];
...
watch(
  ...
  (userId) => {
    if (userId) {
      void metricsStore.fetchStats(userId);
    } else {
      metricsStore.$patch({ stats: null, statsError: null, statsLoading: false });
    }
  },
  { immediate: true }
);
...
const activityWeeks = computed(() => {
  const activity = metricsStore.stats?.activity ?? [];
  if (activity.length === 0) return [];

  const normalized = [...activity];
  const remainder = activity.length % 7;

  if (remainder !== 0) {
    const lastDate = new Date(activity[activity.length - 1].date);
    for (let i = remainder; i < 7; i += 1) {
      lastDate.setDate(lastDate.getDate() + 1);
      normalized.push({ date: lastDate.toISOString().slice(0, 10), count: 0 });
    }
  }
...
const totalActivityCount = computed(() =>
  metricsStore.stats?.activity.reduce((acc, day) => acc + day.count, 0) ?? 0
);

const activityColor = (count: number) => {
  if (count === 0) return 'bg-slate-200/80';
  if (count <= 2) return 'bg-orange-100';
  if (count <= 4) return 'bg-orange-200';
  if (count <= 7) return 'bg-orange-300';
  if (count <= 10) return 'bg-orange-400';
  return 'bg-orange-500';
};

const activityTitle = (date: string, count: number) => {
  const formatted = activityFormatter.format(new Date(date));
  if (count === 0) {
    return `${formatted}: нет активности`;
  }
  if (count === 1) {
    return `${formatted}: 1 взаимодействие`;
  }
  return `${formatted}: ${count} взаимодействия`;
};
...
const handleAccountSave = async () => {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Не удалось сохранить контакты';
    notifications.push({
      title: 'Ошибка',
      description: message,
      tone: 'error'
...
const handleLogout = async () => {
  const previousId = userStore.logout();
  if (previousId) {
